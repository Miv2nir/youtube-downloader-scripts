#a script for downloading a single youtube video coupled with the necessary metadata for the archive
import yt_dlp
import os
import copy
import time
...
playlist_info_json=yt_dlp.YoutubeDL(opts_list).extract_info(playlist_url)
print('playlist_info_json obtained.')
...
if not no_directory:
    for i in dirs:
        try:
            with open(dump_path+'/'+i+'/info.toml','r',encoding="utf-8") as f:
                d=toml.loads(f.read())
...
for i in playlist_info_json["entries"]:
    #print(i)
    urls.append(i['url'])
...
for url in urls:
    ...
    try:
        ...
        if move_to_share:
            ...
            shared_folder_path=dump_path+dir_name
            os.chdir(os.pardir)
            import shutil
            while True:
                try:
        ...
        if handle_crashes:
            ...
            #must be a full path for we do not know where did the script crash exactly in the file
            while True:
                try:
                    with open(dump_path+'_errored_urls.txt','a') as f:
                        f.write(url+'\n')
                    print('OK')
                    break
                except OSError:
...
if is_patreon_exclusive:
    print('All done! Don\'t forget to fill out a patreon_description.md ;)')
else:
    print('All done!')
